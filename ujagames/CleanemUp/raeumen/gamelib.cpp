...
// Individualisierung der Module in config.h
...
// 30.08.2004 uja: File-Control-Blocks eingebaut - man fand heraus, wie man eine Netz-Hiscoreliste baut
...
//                 Code gebummt fuer die ujagames_qt
// 11.09.2004 uja: Erweiterung Viewport, so dass das Mausrad abgefangen wird bei Erhalt der Scrollfaehigkeit  
// 12.09.2004 uja: Zusammenfassung aller konstanten Klassen zur gamelib, Internationalisierung
// ====================================================================================================================
#include "gamelib.h"
...
// Da zum einen zumindest bei der NVidia bei gleicher Canvas-und Viewgroesse stoerende Randpixel auftreten,
...
// Die Scrollbars mussten ausgeblendet werden, zusaetzlich die Reaktion auf das Mausrad. Scrollen selbst muss erhalten bleiben  
// ----------------------------------------------------------------------------------------------------------------------------
viewport::viewport(QWidget* parent, char* name):QCanvasView(parent,name)
{ 
  int xmax=XMAX*DX,ymax=YMAX*DY;             // Daten aus config.h des jeweiligen Programms
...
{ 
  ...
  else if (flag==2) { infile=GPLFILE;  resize(HELPBREITE,320); setCaption((buff+" - "+_LICENSE)); }
...
{ 
  ...
  if (k==0)
  { for (i=0; i<MAXEINTRAG; i++) my_hscliste[i].setValues((500-10*i),"--:--","nemo\n");
...
{ QString lockfile=*hscpfad+"alles_meins.lck"; 
  int gotcha=creat(lockfile,S_IRWXU); 
  return (gotcha>-1);
}

void hiscore::free_lock() { QString lockfile=*hscpfad+"alles_meins.lck"; remove(lockfile); }
// ---------------------------------------------------------------------------------------------
...
{ 
  ...
  if (punkte>=(my_hscliste[MAXEINTRAG-1].punkte)) // Namen holen
  {
    ...
    blabla=_HSCMESS0+buffint+_HSCMESS1;
...
{ 
  ...
  setPaletteBackgroundPixmap(QPixmap(BACKPIX0));
  setMargin(16);
  setSpacing(8);
  ...
  my_text->setPaletteBackgroundPixmap(QPixmap(PAPERBILD));
  my_text->setBackgroundPixmap(QPixmap(BACKPIX2));
  my_text->setFrameShadow(Sunken);
  my_text->setFrameShape(Box);
  my_text->setMargin(8);
  my_text->setMaximumWidth(HSCBREITE);      
  my_text->setMinimumWidth(HSCBREITE);      
  my_text->setMinimumHeight(160);      

  QBrush* my_brush=new QBrush(QColor(BACKGR2),QPixmap(PAPERBILD));
  my_brush->setPixmap(QPixmap(BACKPIX2));
  my_text->setPaper(*my_brush);
...
void hiscore::set_hscpfad(int nr)
{
  ...
  QString buff="";
  QFile file("hiscore.ini");
  if (file.open(IO_ReadOnly)) { for (int i=0; i<=nr; i++) file.readLine(buff,96); file.close(); }
  if (buff.length()>3) *hscpfad=buff.section(';',0,0); 
  *hscdatei=*hscpfad+PROGNAME+".csv"; 

}

// Dieser Slot ist die erste Anlaufstelle, sie reicht den Netzwerkmodus weiter.
// Der Rest wird vom Spielmodul chips/wiese erledigt, der die anderen Parameter beisteuert.
void hiscore::save_settings() { emit rewrite_ini(hscpfad_id); }
