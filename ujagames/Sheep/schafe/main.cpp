...
// Wiesen-Sprites belegen und mit Canvas verbinden:
  ...
  my_wiese->my_bgarray    =new QCanvasPixmapArray(BGSPRITE,NUMBG);
  my_wiese->my_pfeilarray =new QCanvasPixmapArray(PFEILSPRITE,NUMPFEILE);
  my_wiese->my_schafarray =new QCanvasPixmapArray(SCHAFSPRITE,NUMSCHAFE);
  my_wiese->my_wespenarray=new QCanvasPixmapArray(MOSKITOSPRITE,NUMWESPEN);
  my_wiese->my_transparray=new QCanvasPixmapArray(TRANSSPRITE);
  my_wiese->my_goverarray =new QCanvasPixmapArray(GAMEOVERFILE);
  my_wiese->my_pausearray =new QCanvasPixmapArray(PAUSEFILE);

  my_wiese->my_cyclamarray=new QCanvasPixmapArray(CYCLAMSPRITE,NUMCYCLAM);
